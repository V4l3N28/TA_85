...
# 1 se define el decorador que coge el bloque de código de la función que querermos decorar
def login_required(view):
    # 2 utilizamos el módulo functools y la función .wrap para poder trabajar sobre otras funciones
    @functools.wraps(view)
    def wrapped_view(**kwargs):  # 4 Revisamos si el usuario está o no en sesión,
        if g.user is None:
            # 4.1 Si el usuario no está en sesión lo redireccionamos a la página de inicio de sesión
            flash('Primero necesitas iniciar sesión')
            return redirect('/IniciarSesion/')
        # 4.2 Si el usuario está en sesión, lo envía a la página (view) que solicitó
        return view(**kwargs)
...
def dataframe():
  ...
  df = pd.DataFrame(cursor, columns=[
...
def sumatoria_prec(year, df):
  ...
  cursor.execute(
      "SELECT strftime('%m', fecha) as month, SUM(precipitacion) FROM registros WHERE strftime('%Y', fecha)=? GROUP BY month", [year])
  ...
  db_to_dataframe = pd.DataFrame(
      filtered_db, columns=['fecha', 'precipitacion'])
  #Se retorna el dataframe
  return db_to_dataframe


def sumatoria_prec_id(year, df):
  conn = sqlite3.connect('bases_04.db')
  cursor = conn.cursor()
  cursor.execute(
      "SELECT strftime('%m', fecha) as month, SUM(precipitacion) FROM registros WHERE strftime('%Y', fecha)=? GROUP BY month", [year])
  filtered_db = cursor.fetchall()
  db_to_dataframe = pd.DataFrame(
      filtered_db, columns=['idFinca', 'precipitacion'])
  #Se retorna el dataframe
  return db_to_dataframe
...
def promedio_prec(year, df):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT strftime('%m', fecha) as month, AVG(precipitacion) FROM registros WHERE strftime('%Y', fecha)=?", [year])
    filtered_db = cursor.fetchall()
    db_to_dataframe = pd.DataFrame(
        filtered_db, columns=['fecha', 'precipitacion'])
    return db_to_dataframe
...
def iniciosesion():
  ...
  if request.method == 'POST':
    ...
    contraseña_bd = db.execute(
        'SELECT contrasena FROM usuarios WHERE contrasena = ?', (contrasena,)).fetchone()
  ...
  try:
    if request.method == 'POST':
      ...
      if correo == correo2 and contrasena == contrasena2:
        db = get_db()
        db.execute("INSERT INTO usuarios(nombre, apellido, usuario, email, contrasena) VALUES ('%s','%s','%s','%s','%s')" % (
...
def PRONOSTICOS():
  ...
  labels = ['lluvias por encima de lo normal',
            'lluvias dentro de lo normal', 'lluvias poor debajo de lo normal']
...
@login_required
def GENERAL():
  temperaturaMinima = [("1", 12, 13, 24, 10, 17, 21, 32), ("1", 14, 12, 22, 10, 19, 28, 30),
                       ("1", 14, 23, 24, 11, 17, 21, 31), ("1", 10, 19, 27, 10, 16, 21, 35)]
...
def ESTACIONES():
  ...
  if request.method == 'POST':
    select_id = request.form.get('idFinca')
    df = dataframe()
    suma = sumatoria_prec_id(select_idFinca, df)
    labels = list(suma['idFinca'][:])
    values = list(suma['precipitacion'][:])
    return render_template("ESTACIONES.html", labels=labels, values=values)

  temperaturaMaxima = [("productor1", 34),
                       ("productor2", 56), ("productor3", 17)]
